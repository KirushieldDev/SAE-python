...
def player(x: int, y: int, cote: int):
    if invincible:
        rectangle(x, y, x + cote, y + cote, epaisseur="3", couleur="green", tag="player")
    else:
        rectangle(x, y, x + cote, y + cote, epaisseur="3", couleur="red", tag="player")

def player2(x: int, y: int, cote: int):
    rectangle(x, y, x + cote, y + cote, epaisseur="3", couleur="yellow", tag="player2")
...
if __name__ == "__main__":
    ...
    # tailleJoueur2 = 20
    # xJoueur2 = 375
    # yJoueur2 = y_bas_contour - (tailleJoueur // 2)
    # vitesseJoueur2 = 5
    # player(xJoueur2, yJoueur2, tailleJoueur2)
    # listePositionsLignes2 = []
    lstpomme = []
    for i in range(5):
        xpomme=randrange(375,1125)
        ypomme=randrange(200,975)
        pomme(xpomme,ypomme)
        lstpomme.append((xpomme,ypomme))
    ...
    while True and vie>0:
        ...
        # Collisions du Qix
        if x_fantome >= x_droite_contour - 35 or x_fantome <= x_gauche_contour + 35:
            speedXFantome = -speedXFantome

        if y_fantome >= y_bas_contour - 40 or y_fantome <= y_haut_contour + 40:
        ...
        # if touche_pressee("d") and (
        #     (xJoueur2 + (tailleJoueur2 // 2)) < x_droite_contour
        #     and touche_pressee("z") == 0
        #     and touche_pressee("s") == 0
        # ):
        #     xJoueur2 += vitesseJoueur2 # On fait déplacer le joueur à droite
        #     if (
        #         touche_pressee("Space")
        #         and (yJoueur2 + (tailleJoueur2 // 2)) != y_bas_contour
        #         and (yJoueur2 + (tailleJoueur2 // 2)) != y_haut_contour
        #         and (xJoueur2 + (tailleJoueur2 // 2)) != x_gauche_contour
        #         and (xJoueur2 + (tailleJoueur2 // 2)) != x_droite_contour
        #     ): # Si la touche entrée est appuyée et que le joueur est bien positionné
        #         dessin(
        #             xJoueur2 + (tailleJoueur2 // 2),
        #             yJoueur2 + (tailleJoueur2 // 2),
        #             xJoueur2+ (tailleJoueur2 // 2) + vitesseJoueur2,
        #             yJoueur2 + (tailleJoueur2 // 2),
        #         ) # On dessine la ligne qui va suivre le joueur
        #         listePositionsLignes2.append(
        #             (xJoueur2 + (tailleJoueur2 // 2), yJoueur2 + (tailleJoueur2 // 2))
        #         ) # On ajoute les positions des sommets des lignes afin de tracer le polygone
        #         if (
        #             xJoueur2 < x_gauche_contour
        #             or xJoueur2 + tailleJoueur2 > x_droite_contour
        #             or yJoueur2 < y_haut_contour
        #             or yJoueur2 + tailleJoueur2 > y_bas_contour
        #         ):
        #             if len(listePositionsLignes2) > 2:
        #                 if xJoueur2 < x_gauche_contour:
        #                     listePositionsLignes2.append(listePositionsLignes2[0])
        #                 tracerPolygone(listePositionsLignes2)
        #             listePositionsLignes2 = []
        ...
        # if touche_pressee("q") and (
        #     (xJoueur2 + (tailleJoueur2 // 2)) > x_gauche_contour
        #     and touche_pressee("z") == 0
        #     and touche_pressee("s") == 0
        # ):
        #     xJoueur2 -= vitesseJoueur2
        #     if (
        #         touche_pressee("Return")
        #         and (yJoueur2 + (tailleJoueur2 // 2)) != y_bas_contour
        #         and (yJoueur2 + (tailleJoueur2 // 2)) != y_haut_contour
        ...
        # if touche_pressee("z") and ((yJoueur2 + (tailleJoueur2 // 2)) > y_haut_contour):
        #     yJoueur2 -= vitesseJoueur2
        #     if (
        #         touche_pressee("Return")
        #         and (yJoueur2 + (tailleJoueur2 // 2)) != y_bas_contour
        #         and (yJoueur2 + (tailleJoueur2 // 2)) != y_haut_contour
        ...
        #             yJoueur2 + (tailleJoueur2 // 2),
        #             xJoueur2 + (tailleJoueur2 // 2),
        #             yJoueur2 + (tailleJoueur2 // 2) + vitesseJoueur2,
        #         ) # On dessine la ligne qui va suivre le joueur
        #         listePositionsLignes2.append(
        #             (xJoueur2 + (tailleJoueur2 // 2), yJoueur2 + (tailleJoueur2 // 2))
        #         ) # On ajoute les positions des sommets des lignes afin de tracer le polygone
        ...
        # if touche_pressee("s") and ((yJoueur2 + (tailleJoueur2 // 2)) < y_bas_contour):
        #     yJoueur2 += vitesseJoueur2
        #     if (
        #         touche_pressee("Return")
        #         and (yJoueur2 + (tailleJoueur2 // 2)) != y_bas_contour
        ...
        for i in range(len(lstpomme)):
            if positionJoueur == lstpomme[i]:
                lstpomme.remove(lstpomme[i])  # Enlève la pomme touchée de la liste
                debut = time.time()
                invincible = True
        if not invincible:
            if (
        ...
        if invincible:
            invincible_duration += 1 / 60  # Compte le temps en secondes (supposant un rafraîchissement de 60 FPS)
            if invincible_duration >= invincible_timer:
                invincible = False
        mise_a_jour()
# ****************************************************************************************************************************
    if vie < 1:
